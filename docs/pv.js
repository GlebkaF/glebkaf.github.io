  ...
  function ApiPWA() {
    ...
    this.account = function (u, join) {
      if (join) {
        if (
          Defined.use_api == "pwa" &&
          (u.indexOf("http://") > -1 || u.indexOf("https://") > -1)
        )
          return u;
        if (Defined.use_api == "http" && u.indexOf(Defined.localhost) == -1)
          return u;
      }
  ...
  function ApiHttp() {
    ...
    this.bookmark = function (element, add, call) {
      var u =
        Defined.localhost +
        "/bookmark/" +
        (add ? "add" : "remove?uid=" + element.bookmark.uid);
      network.silent(
        this.account(u),
        function (e) {
          call(true);
    ...
    this.account = function (u) {
      if (
        ...
        return u;
      if (
        Defined.use_api == "http" &&
        u.indexOf(Defined.localhost) == -1 &&
    ...
    this.playlist = function (add_url_query, oncomplite, error) {
      var load = function load() {
        ...
        menu.forEach(function (m) {
          network.silent(
            _this.account(m.playlist_url + add_url_query),
            function (json) {
              if (json.list) {
                json.title = Utils.sourceTitle(m.title);
                json.results = Utils.fixList(json.list);
